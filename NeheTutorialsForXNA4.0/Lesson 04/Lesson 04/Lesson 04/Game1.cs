...
namespace Lesson_04
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        //new code
        private float m_triangleRotationAngle, m_triangleRotationSpeed;
        private float m_quadRotationAngle, m_quadRotationSpeed;
        //
        #endregion
        #region Constructors
        public Game1()
        ...
        protected override void Initialize()
        {
            ...
            m_quadRotationAngle = 0.0f;
            m_triangleRotationSpeed = 0.1f;
            m_quadRotationSpeed = 0.05f;

            //So we can see the back of our shapes when rotating
            ...
            m_triangleVertices[0].Position = new Vector3(-triangleSize/2, -triangleSize/2, 0.0f);//bottom left
            m_triangleVertices[1].Position = new Vector3((-triangleSize/2 + triangleSize/2) / 2, +triangleSize/2, 0.0f);//top middle
            m_triangleVertices[2].Position = new Vector3(+triangleSize/2, -triangleSize/2, 0.0f);// bottom right
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            //New code
            m_triangleRotationAngle += m_triangleRotationSpeed *
                (float)gameTime.ElapsedGameTime.TotalMilliseconds;
            m_quadRotationAngle += m_quadRotationSpeed *
                (float)gameTime.ElapsedGameTime.TotalMilliseconds;

            // Create a translation matrix for our shapes based upon their desired position.
            m_translateTriangle = Matrix.CreateRotationX(MathHelper.ToRadians(m_triangleRotationAngle))
            ...
            // two shapes
            Vector3 temp = m_quad.Position - m_trianglePosition;
            temp.X = Math.Abs(temp.X);
            temp.Y = Math.Abs(temp.Y);
            temp.Z = Math.Abs(temp.Z);
            m_translateQuad = Matrix.CreateTranslation(temp)
                * Matrix.CreateRotationY(MathHelper.ToRadians(m_quadRotationAngle)) 
                * Matrix.CreateTranslation(m_trianglePosition);
            //
