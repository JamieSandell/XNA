...
namespace Lesson_07
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        private Matrix m_translateLight;
        private List<Commands> m_commands;
        //
        Texture2D m_texture;
        //New code
        Effect m_effect;
        ...
        protected override void Initialize()
        {
            ...
            m_pyramidRotationSpeed = 50.0f;
            m_cubeRotationSpeed = 20.0f;
            //
            ...
            //New code
            m_commands = new List<Commands>();

            m_lightPosition = new Vector3(0.0f, 0.0f, -7.0f);
            m_light = new Light(ref m_lightPosition, 0.025f, graphics);
            ...
            m_effect.Parameters["xLightPosition"].SetValue(m_light.Position);
            //
            ResetProjection();
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            // Calculate the deltatime and apply it to our rotation angles
            float deltaTime = (float)gameTime.ElapsedGameTime.TotalSeconds;

            m_pyramidRotationAngle += m_pyramidRotationSpeed * deltaTime;
            m_cubeRotationAngle += m_cubeRotationSpeed * deltaTime;
            //
            ...
            // New code
            // Update the position of the light and create the translation matrix for it.
            m_translateLight = Matrix.CreateTranslation(m_light.Position);
            m_effect.Parameters["xLightPosition"].SetValue(m_light.Position);
            //
            //

            // New code
            // Uncomment this code to set a camera at the position of the light
            //Vector3 tempVec = new Vector3();
            //tempVec = m_light.Position;
            //tempVec.Z = tempVec.Z - 1.0f;
            //m_viewMatrix = Matrix.CreateLookAt(m_light.Position, tempVec, Vector3.Up);
            //m_basicEffect.View = m_viewMatrix;
            //m_effect.Parameters["xView"].SetValue(m_viewMatrix);
            //
            //

            // Process the command list and then empty it
            m_light.Update(deltaTime, m_commands);
            m_commands.Clear();
            //
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            for (int i = 0; i < m_basicEffect.CurrentTechnique.Passes.Count; i++)
            {
                ...
                m_basicEffect.LightingEnabled = false;
                // New code
                m_basicEffect.Texture = m_light.Cube.Texture;
                //
                m_basicEffect.World = m_translateLight;
                m_basicEffect.CurrentTechnique.Passes[i].Apply();
                m_light.Draw();
            }
        ...
        private void ProcessInput()
        {
            ...
            if ((Utility.HasKeyBeenPressed(m_keyAltEnter, ref m_prevKeyboardState, ref m_currKeyboardState)))
            {
            ...
            // Process all currently pressed down keys.
            foreach(Keys keys in m_currKeyboardState.GetPressedKeys())
            {
                switch (keys)
                {
                    case Keys.D:
                        m_commands.Add(Commands.MoveRight);
                        break;
                    case Keys.A:
                        m_commands.Add(Commands.MoveLeft);
                        break;
                    case Keys.W:
                        m_commands.Add(Commands.MoveUp);
                        break;
                    case Keys.S:
                        m_commands.Add(Commands.MoveDown);
                        break;
                    case Keys.Up:
                        m_commands.Add(Commands.MoveForward);
                        break;
                    case Keys.Down:
                        m_commands.Add(Commands.MoveBackward);
                        break;
                    default:
                        break;
                }
