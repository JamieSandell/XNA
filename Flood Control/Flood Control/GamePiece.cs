...
namespace Flood_Control
{
    class GamePiece
    {
        ...
        {
            int x = textureOffsetX; // non water non locked piece
            int y = textureOffsetY;

            if (pieceSuffix.Equals("W")) // non locked water piece
                x += PieceWidth + texturePaddingX;
            else if (pieceSuffix.Equals("L")) // locked non water piece
                x += (PieceWidth * 2) + texturePaddingX;
            else if (pieceSuffix.Equals("LW") || pieceSuffix.Equals("WL")) // locked water piece
                x += (PieceWidth * 3) + texturePaddingX;
